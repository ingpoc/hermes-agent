...
def _print_setup_summary(config: dict, hermes_home):
    ...
    print(color("─" * 60, Colors.DIM))
    print()
    print(color("✓ Hermes is ready.", Colors.GREEN, Colors.BOLD))
    print()

    # Provider / model confirmation so operator can verify the right backend is active.
    _model_display = config.get("model") or config.get("default_model") or "default"
    _provider_display = config.get("provider") or "auto-detected"
    _memory_enabled = config.get("memory", {}).get("enabled", True) if isinstance(config.get("memory"), dict) else True
    print(f"   Provider: {color(_provider_display, Colors.CYAN)}")
    print(f"   Model:    {color(_model_display, Colors.CYAN)}")
    print(f"   Memory:   {color('enabled' if _memory_enabled else 'disabled', Colors.CYAN)}")

    # Security summary
    try:
        from agent.sandbox import sandbox_status as _sb_status
        from hermes_constants import ALLOWED_TOOLS as _at
        _sb = _sb_status()
        _net = "network blocked" if _sb.get("network_blocked") else "network open (set HERMES_SANDBOX_NETWORK_DISABLED=1 to block)"
        _sb_label = f"{_sb.get('backend') or 'unavailable'}, {_net}"
        print(f"   Sandbox:  {color(_sb_label, Colors.CYAN)}")
        if _at:
            print(f"   Allowlist:{color(f' {len(_at)} tool(s) permitted', Colors.CYAN)}")
        else:
            print(f"   Allowlist:{color(' disabled (all tools allowed)', Colors.CYAN)}")
    except Exception:
        pass
    print()

    print(color("Suggested first commands:", Colors.CYAN, Colors.BOLD))
    print()
    print(f"   {color('hermes', Colors.GREEN)}              Start a conversation")
    print(f"   {color('hermes hello', Colors.GREEN)}        Run a stack-verification test turn")
